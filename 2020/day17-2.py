...
state = np.zeros((ROWS, ROWS, LAYERS, WLAYERS))
state[7:7+numrows, 7:7+numrows, 7, 7] = initial
    ...
    for x in range(1, ROWS - 1):
        for y in range(1, ROWS - 1):
            for z in range(1, LAYERS - 1):
                for w in range(1, WLAYERS - 1):
                    cube = prevstate[x,y,z,w]
                    if cube == 1:
                        # -1 to not include self
                        neighbors = prevstate[x-1:x+2, y-1:y+2, z-1:z+2, w-1:w+2].sum() - 1
                        if neighbors < 2 or neighbors > 3: state[x,y,z,w] = 0
                    else:
                        neighbors = prevstate[x-1:x+2, y-1:y+2, z-1:z+2, w-1:w+2].sum()
                        if neighbors == 3: state[x,y,z,w] = 1
    prevstate = copy.deepcopy(state)
