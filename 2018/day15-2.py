...
for line in text:
    row += 1
    line = line.strip()
    OGcave.append(list(line))
    col = -1
    for letter in line:
        col += 1
        if letter in 'EG':
            OGcoords.append([row, col])
            OGHP.append(200)
            OGraces.append(letter)

elfpower = 2
while True:
    elfpower += 1
    cave = np.array(OGcave, dtype=object)
    HP = deepcopy(OGHP)
    races = deepcopy(OGraces)
    coords = deepcopy(OGcoords)
    rnd = -1
    while True:
        rnd += 1
        killed = []
        ...
        if len(set(races)) == 1:
            break
    print(rnd * sum(HP))
    if races.count('E') == OGraces.count('E'):
        break
